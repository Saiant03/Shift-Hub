...
- 2026-09-24 — Tests seed storage only on the load opened with `?seed` (the init script drops it via `history.replaceState`) — the old sessionStorage "seeded" guard was sometimes unseen on reload, so the seed overwrote saved data (the salary / shift-reorder reload flakes, ~7/60 reproduced; app not at fault). A second CDP session for seeding broke Playwright's touch/download handling.
